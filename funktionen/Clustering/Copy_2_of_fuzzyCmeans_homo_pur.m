function [ cluster1, cluster2, cluster_zentrum, p] = fuzzyCmeans_homo_pur( cluster_input, vektoren, clusterZentrum, p_fuzzy, numCluster)
    ...
    %rng('default');
    %rng(4);
    p = p_fuzzy; %2=Anzahl Cluster, hier random...
    %p = rand(rows,2);
    ...
    %Zuweisen eines Cluster Label------------------------------------------
       ...
       %sortieren
       %fehler1 = sortrows(homo_fehler(:,2)); %Optimierung über alle Werte
       %fehler2 = sortrows(homo_fehler(:,3));
     ...
     for row = 1:rows   
       %Für Cluster 1
       if homo_fehler(row, 2) > z1_oben %andere Werte können durch einen Ausreißer hoch gezogen werden! Ab wann ist es ein Ausreißer??
           input_daten(row, 1) = 2;
       end
       
       %Für Cluster 2
       if homo_fehler(row, 3) > z2_oben %andere Werte können durch einen Ausreißer hoch gezogen werden! Ab wann ist es ein Ausreißer??
           input_daten(row, 1) = 1;
       end
    ...
    %{
    if i == 1
        sum_fehler1_save = mean(homo_fehler(:,1))
        sum_fehler2_save = mean(homo_fehler(:,2))
    else
        sum_fehler1_save = sum_fehler1
        sum_fehler2_save = sum_fehler2
    end
    
    sum_fehler1 = mean(homo_fehler(:,1))
    sum_fehler2 = mean(homo_fehler(:,2))
    
    if sum_fehler1-sum_fehler1_save == 0
        filter = filter-0.75;
    end
    ...
    %clusterPlot( cluster1, cluster2 , cluster_zentrum, 'iteration cluster');
    
    %Fehler nach Verbesserung der Cluster Plot
    %{
    size_f1 = size(homo_fehler(:,2));
    rows_f1 = size_f1(1);
    rf1 = 1:rows_f1;
    
    size_f2 = size(homo_fehler(:,3));
    rows_f2 = size_f2(1);
    rf2 = 1:rows_f2;
    
    figure('name', 'Homographie Fehler Cluster 1+2 alle Punkte');
    plot(rf1, homo_fehler(:,2), 'r');
    hold on
    %figure('name', 'Homographie Fehler Cluster 2');
    plot(rf2, homo_fehler(:,3), 'b');
    hold off
    %}
