...
fehler_video= clustering_sort(differenz(:,1) ~= 0, 1:4) %von beiden Clustern
%fehler_video= clustering_sort(differenz(:,1) == 0, 1:3);

size_fehler = size(fehler_video);
...
s = scatter(fehler_video(:,2), fehler_video(:,3), 50, fehler_farbe, 'filled');
s.LineWidth = 6.0; 
