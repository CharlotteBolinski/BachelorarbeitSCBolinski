function [ clustering_daten,cluster_zentrum] = Copy_of_fuzzyCmeans_self( input_daten, numCluster )
    ...
    sum_p1 = sum(p(:,1)); %Wahrscheinlichkeiten Cluster 1
    sum_p2 = sum(p(:,2)); %Wahrscheinlichkeiten Cluster 2
    
    zentrum1 = [sum(input_daten(:,1).*p(:,1))/sum_p1 , sum(input_daten(:,2).*p(:,1))/sum_p1];
    zentrum2 = [sum(input_daten(:,1).*p(:,2))/sum_p2 , sum(input_daten(:,2).*p(:,2))/sum_p2];
    ...
    for row = 1:rows
       ...
       distances(row, 3) = sum((input_daten(row,:) - cluster_zentrum(1, :)).^2); %Euklidischer Abstand zu Zentrum 1
       distances(row, 4) = sum((input_daten(row,:) - cluster_zentrum(2, :)).^2); %Euklidischer Abstand zu Zentrum 2
